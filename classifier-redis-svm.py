...
def get_features():
    ...
    _filter_none = functools.partial(filter, lambda words: len(words))

    def _format(sentence, sentiment):
        formatted_sentence = FORMATTER.process(sentence)
        return nltk.wordpunct_tokenize(formatted_sentence), sentiment
    ...
    return _filter_none(pos_features), _filter_none(neg_features), _filter_none(neutral_features)
...
def svmify(*feature_list):
    words = []
    phrases = []
    sentiments = []
    for phrase, sentiment in reduce(add, feature_list, []):
        indexes = []
        for word in phrase:
            if word not in words:
                words.append(word)
            indexes.append(words.index(word))
        phrases.append(indexes)
        sentiments.append(SENTIMENT_MAP[sentiment])
    return np.array(words), np.array(phrases), np.array(sentiments)

def main():
    # Fetch features from phrases
    pos_features, neg_features, neutral_features = get_features()
    words, phrases, sentiments = svmify(pos_features, neg_features, neutral_features)

    cutoff = len(phrases) * 3/4

    import ipdb; ipdb.set_trace()
    clf = svm.SVC()
    clf.fit(phrases[:cutoff], sentiments[:cutoff]) 
    print clf.score(phrases[cutoff:], sentiments[cutoff:]) 




    # loop and classify incoming data through ZMQ
