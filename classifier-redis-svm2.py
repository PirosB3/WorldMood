...
def get_features():
    ...
    neutral_sentences = list(client.smembers('sentiment-analysis-3:neutral'))
...
def evaluate_classifier(valid_features, pos_features, neg_features, neutral_features):
    ...
    classifier.train(map(_extract_valid, train_set))
    print "Built classifier."

    test_set_extracted = map(_extract_valid, test_set)
    actual_features = [s[0] for s in test_set_extracted]
    actual_labels = [s[1] for s in test_set_extracted]

    print classification_report(
        actual_labels,
        classifier.batch_classify(actual_features)
    )
